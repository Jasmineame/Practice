...
for arg in "$@"; do
  case "$arg" in
    --train_thres) MODE="thres" ;;
    --do_oracle)   MODE="oracle" ;;
  esac
done
...
for M in "${Ms[@]}"; do
    for D in "${datasets[@]}"; do
      ...
      if [ -f "$data_file" ]; then
        echo "Dataset already exists at $data_file, skipping preparation."
      else
    ...
    for phi in "${phis[@]}"; do
      ...
      for width in "${widths[@]}"; do
        ...
        python scripts/detector_voting_tp_autothres.py \
          ...
          --num_subsample 2 \
          ${TRAIN_ARG} \
          --rewrite_model_name "Qwen/Qwen3.5-4B" \
          --local_files_only True \
          --cache_dir /root/project/Practice/cache/ \
