...
from data_builder_sentence import count_sentences
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resample_times', type=int, default=100)
    ...
    for i in tqdm(range(n_samples), desc="Processing samples"):
        ...
        if args.train_thres:
            ...
            for idx in range(len(rewrite_text)):
                num_rewrite_sentence, rewrite_sens_list = count_sentences(rewrite_text[idx])
                # if num_rewrite_sentence < args.width:
                #     rewrite_scores = []
                #     for j in range(num_rewrite_sentence):
                #         stat = llm_detector.score(rewrite_sens_list[j])
                #         rewrite_scores.append(stat)
                # else:
                rewrite_votes = [[] for _ in range(num_rewrite_sentence)]
                for resample_time in tqdm(range(args.resample_times), desc="resampling"): 
                    int_list = rng.integers(0, num_rewrite_sentence, 2)
                    print("构建阈值中:这次抽取到的随机区间是:", int_list)
                    prediction_score = llm_detector.score(" ".join(rewrite_sens_list[min(int_list):max(int_list)]))
                    for vote_idx in range(min(int_list), max(int_list) + 1):
                ...
                #     for window_start in range(0, max(1, num_rewrite_sentence - args.width + 1), 1):
                #         with torch.no_grad():
                #             prediction_score = llm_detector.score(" ".join(rewrite_sens_list[window_start:(window_start + args.width)]))
                #         for vote_idx in range(window_start, window_start + args.width):
                #             rewrite_votes[vote_idx].append(prediction_score)
                #     rewrite_scores = [sum(sub_list) / len(sub_list) for sub_list in rewrite_votes]
                # all_rewrite_score.extend(rewrite_scores)
                # rewrite_scores = all_rewrite_score
        ...
        for resample_time in tqdm(range(args.resample_times), desc="resampling"):
            int_list = rng.integers(0, num_sentence, 2)
            print("检测中:这次抽取到的随机区间是:", int_list)
            prediction_score = llm_detector.score(" ".join(sens_list[min(int_list):max(int_list)]))
            for vote_idx in range(min(int_list), max(int_list) + 1):
        ...
        if args.train_thres:
            ...
            results.append({
                ...
                "null_scores": all_rewrite_score, "best_thres": thres_i,
