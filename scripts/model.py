...
# 接着执行您原来的代码
def from_pretrained(cls, model_name, kwargs, cache_dir, **extra_kwargs):
    huggingface_style_path = os.path.join(cache_dir, f"models--{model_name.replace('/', '--')}")
    old_local_path = os.path.join(cache_dir, 'local.' + model_name.replace("/", "_"))

    if os.path.exists(huggingface_style_path):
        return cls.from_pretrained(huggingface_style_path, **kwargs, **extra_kwargs)
    elif os.path.exists(old_local_path):
        return cls.from_pretrained(old_local_path, **kwargs, **extra_kwargs)
    else:
        # 尝试走原始逻辑，例如下载或远程查找
        return cls.from_pretrained(model_name, **kwargs, cache_dir=cache_dir, **extra_kwargs)

# def from_pretrained(cls, model_name, model_kwargs, cache_dir, **kwargs):
#     # use local model if it exists
#     local_path = os.path.join(cache_dir, 'local.' + model_name.replace("/", "_"))
#     if os.path.exists(local_path):
#         return cls.from_pretrained(local_path, **model_kwargs, **kwargs)
#     return cls.from_pretrained(model_name, **model_kwargs, cache_dir=cache_dir, **kwargs)
...
def load_model(model_name, device, cache_dir, torch_dtype=None, local_files_only=False):
    ...
    model = from_pretrained(AutoModelForCausalLM, model_fullname, model_kwargs, cache_dir, local_files_only=local_files_only)
...
def load_tokenizer(model_name, cache_dir, local_files_only=False):
    model_fullname = get_model_fullname(model_name)
    optional_tok_kwargs = {}
    ...
    base_tokenizer = from_pretrained(AutoTokenizer, model_fullname, optional_tok_kwargs, cache_dir=cache_dir, local_files_only=local_files_only)
